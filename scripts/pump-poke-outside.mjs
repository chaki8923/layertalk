/**
 * 止まった webview は「外から来たソケットのデータ」で起きるか。
 *
 *   node scripts/pump-poke-outside.mjs [遅延秒=75]
 *
 * `LAYERTALK_OVERLAY_SELFTEST=pump` のプローブが張っている固定名のチャンネルへ、
 * **アプリの外から** broadcast を投げる。コメントは postgres_changes、つまり同じ
 * realtime ソケットの受信フレームで届くので、ここで起きるなら購読は隠れた
 * webview のままでも成立する。起きないなら購読の置き場所を変えるしかない。
 */
