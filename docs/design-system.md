...
> **例外はフキダシモードだけ。** こちらは不透明な白い板を敷きます（`--lt-bubble-*`）。
> スライドを隠すのを承知のうえで可読性を取るモードで、発表者が自分で選んだときにしか
> 出ません。既定は横流し（`displayMode: "flow"`）のままで、そちらは縁取り文字を維持します。
...
オーバーレイのフキダシ専用色。**ライト/ダークで反転させません**（`prefers-color-scheme` の
ブロックには書かない）。スライドの上に乗るので、発表者の Mac の外観設定で色が変わると
明るいスライドの上で板ごと消えてしまうためです:

| トークン | 値 | 用途 |
|---|---|---|
| `--lt-bubble-bg` | `#FFFFFF` | フキダシの地。不透明（`--lt-surface` を使わない） |
| `--lt-bubble-text` | `#12161F` | フキダシの文字 |
| `--lt-bubble-border` | `rgb(15 20 32 / .14)` | フキダシの枠としっぽの外周 |
...
**演出用の長い動き**（原則の例外）:
- 横流れコメント: `linear`、文字数比例で 6–12 秒。縦位置は**空きレーンからの抽選**で、
  さらにレーン内で `LANE_GAP_PX / 2` まで揺らす（グリッドに整列させない）。
  **揺れ幅をこれ以上広げると隣のレーンの文字と重なる。**
- フキダシコメント: 9秒固定。3レーンを順番に使い、スタンプと同じ
  `RISE_EASE` で画面下から上へ流す。文字の実寸を測って完全に画面外へ出す。
  不透明な板なので、同じレーンの先行が抜けるまで発射を遅らせる（重なると下が読めない）。
...
| `--shadow-bubble` | オーバーレイのフキダシ（スライドから浮かせる） |
...
フキダシモードだけは縁取りを使わず、不透明な板に乗せます（`.lt-overlay-bubble`）。
**この2つのクラスは併用しません** — 白い板の上に白文字＋黒縁を置くと読めなくなります。
板の幅は `display: inline-block` の shrink-to-fit と `max-width: 100%` で決まります。
つまり短文では文字に吸い付き、長文ではレーンの幅まで伸びてから折り返して縦に伸びる。
140文字（`COMMENT_MAX_LENGTH`）でも省略記号で切りません（全文が読めることを優先）。
しっぽは擬似要素の三角2枚で、枠線ぶんの三角に塗りの三角を少し小さく重ねて作ります。

参加QRカードは白い紙として置きます。透過オーバーレイの上でも読み取れるよう背景は必ず
不透明な白、`marginSize` でクワイエットゾーンを確保し、下に参加コードを等幅で添えて
読み取れない人の逃げ道を残します。スライド上では左下（右端は質問パネルの領域）に置き、
コントロール窓のトグルでのみ出し入れします。
**QR も文字もカードの中心に揃えます。** ブランドのロゴを出すときだけ、ロゴを左端に敷いて
「ロゴ＋文字」をひとかたまりとして中央に見せます（文字だけを右へ寄せた状態でロゴを消すと、
文字がカード中心から外れる）。寸法は `overlay_render.rs` の `join_card_text_frame` が正で、
ここには書きません — スライドに出る実物はネイティブ（CALayer）描画で、`JoinQrCard.tsx` は
コントロール窓のプレビュー専用です。
