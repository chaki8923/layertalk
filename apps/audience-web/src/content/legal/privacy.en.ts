...
export function privacyContentEn(config: {
  ...
  return {
    ...
    sections: [
      ...
      { id: "storage", title: "8. Cookies and local storage", paragraphs: ["Cookies or local storage in the browser or app are used for authentication state, display settings, room information, offline entitlement checks, and similar purposes. Cookies are not used for advertising.", "When you register a destination and enable automatic sending, this Mac sends the join URL and a fixed message directly to your selected Slack or Microsoft Teams destination on presentation start. Test messages and manual resends also post to the selected destination. Webhook URLs and notification settings are stored in this Mac's Keychain per account and room, and are never sent or synchronized to LayerTalk servers. Turn automatic sending off and save to stop it; deleting a destination removes its settings and URL from Keychain. Retention and deletion of posted messages follow the destination service and your organization settings."] },
