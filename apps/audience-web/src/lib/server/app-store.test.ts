...
describe("validateEventPassTransaction", () => {
  it("accepts a bound non-renewing subscription transaction", () => {
  ...
  it.each([
    ...
    [{ type: Type.CONSUMABLE }, "Event Pass must be a non-renewing subscription"],
    [{ type: Type.NON_CONSUMABLE }, "Event Pass must be a non-renewing subscription"],
    [{ type: Type.AUTO_RENEWABLE_SUBSCRIPTION }, "Event Pass must be a non-renewing subscription"],
