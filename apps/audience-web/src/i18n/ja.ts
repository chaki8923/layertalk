...
export const ja = {
  meta: {
    // title は検索結果でいちばん効くので、キャッチコピーではなく検索される語（コメント・質問・流す・Mac・
    // プレゼン）を入れる。キャッチコピー「会場の声を、スライドの上へ。」は LP の H1 が持っている。
    // 狙う検索語: 「プレゼン 盛り上げる」「LT 盛り上げる」「発表 盛り上げる」「スライド コメント 流す」。
    // 方法を探す検索は解説記事（/guides）が受け、LP は製品名と機能の検索を受ける。
    title: "LayerTalk | プレゼン・LTを盛り上げる、スライドにコメントを流すMacアプリ",
    description: "プレゼンやLT、発表を盛り上げるMac用アプリ。発表中のスライドに、観客のコメント・質問・スタンプをリアルタイムで流せます。観客はQRか6文字コードで参加、アプリも登録も不要。PowerPoint・Keynote・Canvaなどそのまま使えます。",
  ...
  /**
   * 通報（App Store 1.2 の「不適切な内容を報告する手段」）。
   *
   * **Event Pass の有無に関わらず出す。** NG ワードと承認制は有料機能だが、通報と
   * 問い合わせ先は無料ルームでも必ず要る。理由は `report_content` の migration を参照。
   */
  report: {
    open: "この投稿を通報",
    openStamp: "このスタンプを通報",
    title: "通報する理由を選んでください",
    body: "発表者に通知され、内容を確認します。誰が通報したかは相手に伝わりません。",
    reasons: {
      offensive: "不適切・攻撃的な内容",
      harassment: "特定の人への嫌がらせ",
      spam: "スパム・宣伝",
      other: "その他",
    },
    cancel: "やめる",
    ...
    /** 通報だけで消えない場合の逃げ道。1.2 は「連絡先を公開していること」も求める。 */
    contact: "運営に問い合わせる",
    /** カスタムスタンプは長押しで通報する。バーに通報ボタンを並べると列が2倍になるため。 */
    stampHint: "カスタムスタンプは長押しで通報できます",
  },
